...
public interface DestaqueRepository extends JpaRepository<Destaque, Long> {
    ...
    @Query("SELECT DISTINCT d FROM Destaque d LEFT JOIN FETCH d.stories WHERE d.id = :destaqueId")
    Optional<Destaque> findByIdWithStories(@Param("destaqueId") Long destaqueId);
    ...
    @Query("SELECT DISTINCT d FROM Destaque d LEFT JOIN FETCH d.stories WHERE d.usuario.id = :usuarioId")
    ...
    Page<Storie> findStoriesByDestaqueId(@Param("destaqueId") Long destaqueId, Pageable pageable);
}
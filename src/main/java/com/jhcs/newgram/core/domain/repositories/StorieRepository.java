...
public interface StorieRepository extends JpaRepository<Storie, Long> {
    ...
    List<Storie> findByAutorIdAndDataExpiracaoAfterOrderByDataCriacaoDesc(Long autorId, Date agora);
    ...
    List<Storie> findStoriesDeSeguidosAtivos(@Param("usuarioId") Long usuarioId, @Param("agora") Date agora);
    ...
    @Query("SELECT s FROM Storie s WHERE s.id NOT IN (SELECT v.id FROM Usuario u JOIN u.visualizadoPor v WHERE u.id = :usuarioId) AND s.autor.id = :autorId AND s.dataExpiracao > :agora")

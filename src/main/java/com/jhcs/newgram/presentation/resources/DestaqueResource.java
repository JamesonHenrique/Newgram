...
public class DestaqueResource {
    ...
    public ResponseEntity<Page<DestaqueResponseDTO>> listarDestaquesPorUsername(
        ...
        int total = destaques.size();
        int start = (int) pageable.getOffset();
        int end = Math.min(start + pageable.getPageSize(), total);
        List<DestaqueResponseDTO> conteudo = start >= total ? List.of() : destaques.subList(start, end);
    ...
    public ResponseEntity<Page<StorieResponseDTO>> listarStoriesPorDestaque(
        ...
        int start = (int) pageable.getOffset();
        int end = Math.min(start + pageable.getPageSize(), total);
        List<StorieResponseDTO> conteudo = start >= total ? List.of() : stories.subList(start, end);
        return ResponseEntity.ok(new PageImpl<>(conteudo, pageable, total));
    }
    ...
    public ResponseEntity<ContagemStoriesDTO> contarStoriesPorDestaque(
        ...
        return ResponseEntity.ok(new ContagemStoriesDTO(quantidade));
    }
}